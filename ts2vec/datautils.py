...
def load_DORA_KA51AG8742(dataset):
    train_array, test_array = [np.load(f'datasets/DORA/X_{file_}.npz')['sensor'] for file_ in ['train', 'test']]
    train_labels, test_labels = [np.load(f'datasets/DORA/y_{file_}.npy') for file_ in ['train', 'test']]
...
def load_DORA_(dataset):
    train_array = np.load(f'datasets/DORA/{dataset}.npz',)['sensor'].astype(np.float64)
    print(f'Load DORA Dataset : {train_array.shape}')
