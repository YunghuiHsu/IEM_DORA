...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', type=str, default=None, help='Whether to load model checkpoint, "Fill in your model checkpoint path"')
    parser.add_argument('--eval_only', action="store_true", help='Whether to perform evaluation without training')
    ...
    elif args.loader == 'DORA':
        task_type = 'classification'
        if args.dataset == 'dora':                          # training on all vehicle model 
            train_data =  datautils.load_DORA(args.dataset)   
        elif args.dataset == 'dora_KA51AG8742':             # finetune on one vehicle model 
            train_data, train_labels, test_data, test_labels = datautils.load_DORA_KA51AG8742(args.dataset)
    ...
    if args.resume:
        ...
        epoch_resume = os.path.split(model_path)[-1].lstrip('model_').rstrip('.pkl')
        print(f'Load {model_path}, resume epoch : {epoch_resume}')
